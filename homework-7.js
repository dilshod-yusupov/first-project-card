function printWeather(cityName, temperature) {
  console.log(`Сейчас в ${cityName} ${temperature} градусов по Цельсию`);
}
// Тестируем логику:
...
function fillGasTank(fuelType, tankCapacity, currentFuelLevel) {
  if (currentFuelLevel < CRITICAL_FUEL_LEVEL) {
    ...
    console.log(
      `Критический уровень топлива! До полного бака нужно залить еще ${fuelNeeded} л.`,
    );
  } else if (
    currentFuelLevel >= CRITICAL_FUEL_LEVEL &&
    currentFuelLevel < tankCapacity
  ) {
    const fuelNeeded = tankCapacity - currentFuelLevel;
    console.log(
      `Уровень топлива в норме. В бак поместится еще ${fuelNeeded} л.`,
    );
  } else {
